...
void LCD_write(char data, char RS)
{
   ...
   us_timer(1); /* 1ms（40ns以上）の時間待ち */
   PBDR |= 0x01; /* Eに1をセットする */
   us_timer(1); /* 1ms（230ns以上）の時間待ち */
   PBDR &= 0xFE; /* Eに0をセットする */
}
...
void LCD_init(void)
{
   ms_timer(20); /* 20ms（15ms以上）の時間待ち */
   LCD_write(0x30, 0); /* 8bitモード設定 */
   ms_timer(5); /* 5ms（4.1ms以上）の時間待ち */
   LCD_write(0x30, 0); /* 8bitモード設定 */
   us_timer(200); /* 5ms（100us以上）の時間待ち */
   LCD_write(0x30, 0); /* 8bitモード設定 */
   ms_timer(5); /* 5ms（4.1ms以上）の時間待ち */
   LCD_write(0x20, 0); /* 4bitモード設定 */
   us_timer(50); /* 5ms（40us以上）の時間待ち */
