void speech_packet_send(unsigned char *packet)
{
   ...
   speech_packet -> ip_total_length = 20 + 8 + 160; /* IPデータグラムの長さ */
   ...
   speech_packet -> udp_length = 8 + 160; /* UDPセグメントの長さ */
   ...
   sum += ones_complement_sum(packet, 34, (8 + 160)); /* 1の補数和（UDPセグメント） */
   ...
   packet_send(packet, (14 + 20 + 8 + 160)); /* パケットを送信する */
}
