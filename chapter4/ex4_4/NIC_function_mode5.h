unsigned char NIC_read(unsigned char address)
{
   ...
   ptr = (volatile unsigned char *)0x200000; /* 0x200000番地を指し示すポインタ */
   data = ptr[address]; /* (0x200000 + address)番地のデータを読み取る */
...
void NIC_write(unsigned char address, unsigned char data)
{
   ...
   ptr = (volatile unsigned char *)0x200000; /* 0x200000番地を指し示すポインタ */
   ptr[address] = data; /* (0x200000 + address)番地にデータを書き込む */
}
